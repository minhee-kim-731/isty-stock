import { now, bumpVersionStmt } from "./db.js";
 ...
 * 이미 제품이 있으면 아무것도 하지 않으므로 몇 번을 호출해도 안전하다.
 */
export async function seedIfEmpty(db) {
  const { n } = await db.prepare("SELECT COUNT(*) AS n FROM products").first();
  if (n > 0) return false;
  ...
  for (const [id, brand, name, cost] of PI00786492) {
    stmts.push(
      db
        .prepare(
          "INSERT INTO products (id, brand, name, cost, stock, min_stock, created_at) VALUES (?, ?, ?, ?, 0, 2, ?)"
        )
        .bind(id, brand, name, cost, ts)
    );
  }

  // 위 INSERT 로 만들어진 입고 건의 id 를 SELECT 로 집어서 품목과 원장을 붙인다.
  const ibId = "(SELECT id FROM inbounds WHERE ref = 'PI00786492' ORDER BY id DESC LIMIT 1)";
  for (const [id, , , , qty] of PI00786492) {
    stmts.push(
      db
        .prepare(`INSERT INTO inbound_items (inbound_id, product_id, qty) VALUES (${ibId}, ?, ?)`)
        .bind(id, qty),
      db
        .prepare(
          `INSERT INTO stock_moves (product_id, delta, reason, ref, actor, created_at)
           VALUES (?, ?, 'inbound', 'PI00786492', 'brand', ?)`
        )
        .bind(id, qty, ts),
      db.prepare("UPDATE products SET stock = stock + ? WHERE id = ?").bind(qty, id)
    );
  }
  ...
  stmts.push(
    db
      .prepare("INSERT INTO activity (actor, text, created_at) VALUES ('brand', ?, ?)")
      .bind(`입고 PI00786492 · ${PI00786492.length}개 품목 ${units}개 등록`, ts),
    bumpVersionStmt(db)
  );
